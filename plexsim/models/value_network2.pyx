...
#             double energy  = self._H[node] * self._deref(states)[node]

#         if self._nudges.find(node) != self._nudges.end():
#             energy += self._nudges[node] * self._deref(states)[node]
...
#         cdef state_t proposal = self._deref(states)[node]
#         cdef:
#             state_t start
#             rule_t rule_pair
#             size_t j
...
#         cdef state_t tmp = self._deref(states)[node]
#         self._deref(states)[node] = state
#         cdef:
#             double energy = self._energy(node)
#             double p = exp(self._beta * energy)

#         self._deref(states)[node] = tmp
#         return p
...
#             state_t cur_state= self._deref(states)[node]
...
# #         cdef state_t neighbor_state, state_at_distance, node_state = deref(states)[node]
