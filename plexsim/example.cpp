...
public:
  virtual ~Property() {}
  virtual Property &operator=(const T &f) {
    value = f;
    return *this;
  }
...
public:
  ...
  Adjacency(py::object graph) {
    ...
    this->graph = graph;
    this->nNodes =
        static_cast<size_t>(py::int_(graph.attr("number_of_nodes")()));
    this->nodeids = xt::arange(this->nNodes);
    return;
  }

  Neighbors operator[](nodeID_t node) { return this->adj[node].neighbors; };
};

template <typename IMPL> class Model_ {
public:
  FOENA states;
  FOENA newstates;
  ...
  Model_(py::object graph, FOENA agentStates,
         // agentStates_t agentStates,
         size_t sampleSize) {
  ...
  void swap_buffers() { std::swap(this->states, this->newstates); }
...
// TODO: There is someothing going wrong with a caster of xtensor with this
// model
// The u-fvisibility-inlines-hiddenp-down casting seems to work fine and the
// binding requires less boiler plate code
...
public:
  ...
  Potts_(py::object graph, FOENA agentStates = FOENA({0, 1}),
         size_t sampleSize = 0, double t = 1.)
      : Model_(graph, agentStates, sampleSize) {
    this->t = t;
  };
  ...
  void step(nodeID_t node) {
    ...
    std::transform(this->adj[node].begin(), this->adj[node].end(), idx_.begin(),
                   key_selector);
    std::transform(this->adj[node].begin(), this->adj[node].end(), w_.begin(),
                   value_selector);
...
public:
  ...
  // constructor
  Model(py::object graph, agentStates_t agentStates = agentStates_t(0, 1),
        string nudgeType = "constant", string updateType = "async",
        size_t sampleSize = 0) {
  ...
  void create_adj(py::object graph) {
    ...
    this->graph = graph;
    this->nNodes =
        static_cast<unsigned int>(py::int_(graph.attr("number_of_nodes")()));
  ...
  virtual void swap_buffers() { std::swap(this->states, this->newstates); }
...
public:
  ...
  Potts(py::object graph, agentStates_t agentStates = agentStates_t(1, 0),
        double t = 1, string nudgeType = "constant",
        string updateType = "async", size_t sampleSize = 0)
      : Model(graph, agentStates, nudgeType, updateType, sampleSize) {
    this->t = t;
  };
  ...
  void step(nodeID_t node) {
    ...
    // nodeState_t nstate;
    // double weight;
    // #pragma omp parallel for default(none) reduction(-:cEn, fEn) shared(tmp,
    // nodeState, proposal)
    ...
    for (auto neighbor : tmp) {
      for (size_t j = 0; j < 2; j++) {
        energies[j] -=
            neighbor.second *
            this->hamiltonian(check[j], this->states[neighbor.first]);
      }
    }
  ...
  xarrd magnetize(xt::pyarray<double> &temps, size_t nSamples, double match) {
    ...
    for (size_t tidx = 0; tidx < temps.size(); tidx++) {
      ...
      if (tidx >= 1) {
        xt::view(results, 2, tidx) = (results[1, tidx] - results[1, tidx - 1]) /
                                     (temps[0, tidx] - temps[0, tidx - 1]);
      }
    }
    // if (match >= 0){
    //    py::object optimize  = py::module::import("scipy.optimize");
    //    py::list param = optimize.attr("curve_fit")(sigmoid, temps,
    //    xt::view(results, 1), maxfev = 10000);

    //    double t = optimize.attr("fmin")(sigmoidOps, x0 = 0, args = (param[0],
    //    match)); tcopy.t = t;
...
public:
  ...
  PottsFast(py::object graph, agentStates_t agentStates = agentStates_t({0, 1}),
            string nudgeType = "constant", string updateType = "async",
            size_t sampleSize = 0, double t = 0) {
...
template <class Base = Model> class PyModel : public Base {
public:
...
template <class Base = Potts> class PyPotts : public PyModel<Base> {
public:
...
PYBIND11_MODULE(example, m) {
  ...
  py::class_<Potts_>(m, "PD")
      .def(py::init<py::object, xt::pyarray<double>,
                    // agentStates_t,
                    size_t, double>(),
           "graph"_a, "agentStates"_a = agentStates_t({0, 1}),
           "sampleSize"_a = 0, "t"_a = 1.)
      .def("step", &Potts_::step)
      .def("simulate", &Potts_::simulate)
      .def("sampleNodes", &Potts_::sampleNodes)
      .def_property(
          "graph", [](const Potts_ &self) { return self.adj.graph; },
          [](const Potts_ &self) {});

  py::class_<Model, PyModel<>>(m, "Model")
      .def(py::init<py::object, agentStates_t, string, string, size_t>(),
           "graph"_a, "agentStates"_a, "nudgeType"_a = "constant",
           "updateType"_a = "async", "sampleSize"_a = 0)
      .def_readwrite("graph", &Model::graph)
      .def("sampleNodes", &Model::sampleNodes)
      .def("simulate", &Model::simulate)
      .def_property(
          "nudgeType",
          [](const Model &self) { return static_cast<string>(self.nudgeType); },
          [](Model &self, string s) { self.nudgeType = s; })
      .def_property(
          "updateType",
          [](const Model &self) {
  ...
  py::class_<PottsFast>(m, "PottsFast")
      .def(py::init<py::object, agentStates_t, string, string, size_t>(),
           "graph"_a, "agentStates"_a = agentStates_t(1, 0),
           "nudgeType"_a = "constant", "updateType"_a = "async",
           "sampleSize"_a = 0)
      ...
      .def_property(
          ...
          [](PottsFast &self, string s) { self.updateType = s; })
  ...
  py::class_<Potts, PyPotts<>>(m, "Potts")
      .def(
          py::init<py::object, agentStates_t, double, string, string, size_t>(),
          "graph"_a, "agentStates"_a = agentStates_t({0, 1}), "t"_a = 1.,
          "nudgeType"_a = "constant", "updateType"_a = "async",
          "sampleSize"_a = 0)
      ...
      .def("magnetize", &Potts::magnetize,
           "temps"_a =
               static_cast<xt::pyarray<double>>(xt::logspace(-3, 1, 10)),
           "nSamples"_a = 1000, "match"_a = -1)
      .def_property(
          "t", [](const Potts &self) { return static_cast<double>(self.t); },
